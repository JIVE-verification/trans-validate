...
int stride;}layer_t;
// define functions
layer_t *init_pooling(const char *name,const int ndim,const int *input_shape,const int ksize,const int pad,const int stride);
...
layer_t *init_pooling(const char *name,const int ndim,const int *input_shape,const int ksize,const int pad,const int stride)
{
  (((void )(sizeof(((ndim > 0 && input_shape && ksize > 0 && pad >= 0 && stride > 0?1 : 0))))) , ((
{
    if (ndim > 0 && input_shape && ksize > 0 && pad >= 0 && stride > 0) 
      ;
     else 
      __assert_fail("ndim>0 && input_shape && ksize>0 && pad>=0 && stride>0","ysh329_OpenMP-101_pooling.c",49,__PRETTY_FUNCTION__);
  })));
  ...
  (((void )(sizeof(((l?1 : 0))))) , ((
{
    if (l) 
      ...
      __assert_fail("l","ysh329_OpenMP-101_pooling.c",90,__PRETTY_FUNCTION__);
  })));
  ...
  (((void )(sizeof(((l?1 : 0))))) , ((
{
    if (l) 
      ...
      __assert_fail("l","ysh329_OpenMP-101_pooling.c",125,__PRETTY_FUNCTION__);
  })));
  int h_offset = -l -> pad;
  int w_offset = -l -> pad;
  dummyMethod1();
        ...
        for (int j = 0; j <= l -> output_shape[3] - 1; j += 1) {
          ...
          for (int n = 0; n <= l -> ksize - 1; n += 1) {
            for (int m = 0; m <= l -> ksize - 1; m += 1) {
              ...
              float val = valid != 0?l -> input[in_idx] : - 3.40282346638528859811704183484516925e+38F;
              max = (val > max?val : max);
...
void destroy_pooling(layer_t *l)
{
  (((void )(sizeof(((l && l -> input && l -> output && l -> input_shape && l -> output_shape?1 : 0))))) , ((
{
    if (l && l -> input && l -> output && l -> input_shape && l -> output_shape) 
      ;
     else 
      __assert_fail("l && l->input && l->output && l->input_shape && l->output_shape","ysh329_OpenMP-101_pooling.c",172,__PRETTY_FUNCTION__);
  })));
  if (l -> input) 
    free((l -> input));
  l -> input = ((void *)0);
