...
// v.objects = malloc(sizeof(struct spaceball*)*100); // bounds of 
  ...
  for (int i = sx; i <= ex; i += 1) {
    for (int j = sy; j <= ey; j += 1) {
      for (int k = sz; k <= ez; k += 1) {
// struct volume* n = malloc(sizeof(struct volume));
        v -> x[counter] = i;
//array pts to each coord counter
        v -> y[counter] = j;
// array will point to x,y,z malloc with sizeof float
        v -> z[counter] = k;
// v->mass[i] = 1; //
...
#pragma omp parallel for reduction (+:mass_sum,wx,wy) firstprivate (length)
//#pragma rose_outline
...
// include our struct instea//d of &v this will be length of our array
// Generate output
  float cx;
  float cy;
