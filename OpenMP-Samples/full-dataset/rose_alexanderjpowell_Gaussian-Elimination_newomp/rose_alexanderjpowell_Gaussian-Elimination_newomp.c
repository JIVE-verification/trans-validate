...
//create matrix
  ...
  dummyMethod3();
  for (i = 0; i <= 4; i += 1) {
     *(matrix + i) = (malloc(sizeof(double ) * (5 + 1)));
  }
  dummyMethod4();
/*
	//test input
...
void rowReduce()
{
  ...
  double max;
  int maxRow;
  for (pivot = 0; pivot <= 3; pivot += 1) {
//printf("pivot %d\n", pivot);
//PIVOT THREAD
    max = matrix[pivot][pivot];
//*(*(matrix + pivot) + pivot)
    maxRow = pivot;
    for (i = pivot + 1; i <= 4; i += 1) {
      temp = matrix[i][pivot];
//(*(*(matrix + i) + pivot)
      if (temp > max) {
        max = temp;
...
//	shared (matrix, X, B, n) num_threads(4)
    for (row = pivot + 1; row <= 4; row += 1) {
      ratio = matrix[row][pivot] / matrix[pivot][pivot];
      dummyMethod1();
//printf("t1: row = %d, piv = %d, ratio = %f\n", row, pivot, ratio);
      
#pragma omp parallel for private (i) firstprivate (ratio)
//#pragma rose_outline
      for (i = pivot; i <= 5; i += 1) {
