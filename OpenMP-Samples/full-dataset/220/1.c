...
void parameters(int argc,char **argv)
{
  ...
  if (argc >= 2) {
    ...
    if (MATRIX_SIZE < 1 || MATRIX_SIZE > 20000) {
...
void initializeInputValues()
{
  ...
  printf("Initializing...\n");
  dummyMethod3();
  for (col = 0; col <= MATRIX_SIZE - 1; col += 1) {
    for (row = 0; row <= MATRIX_SIZE - 1; row += 1) {
...
void gaussianEliminationUsingOpenMp()
{
  ...
  int column;
  float multiplier;
  for (iterator = 0; iterator <= MATRIX_SIZE - 1 - 1; iterator += 1) {
    dummyMethod1();
    for (row = iterator + 1; row <= MATRIX_SIZE - 1; row += 1) {
...
//#pragma rose_outline
      for (column = iterator; column <= MATRIX_SIZE - 1; column += 1) {
...
/* Back substitution */
  for (row = MATRIX_SIZE - 1; row >= 0; row += -1) {
    RESULT[row] = VECTOR[row];
    for (column = MATRIX_SIZE - 1; column >= row + 1; column += -1) {
...
int main(int argc,char **argv)
{
  ...
  struct timezone dummyTimeZone;
  unsigned long long executionStartTime;
  unsigned long long executionEndTime;
