...
long get_nanosecs(struct timespec start_time,struct timespec end_time)
{
  ...
  if (end_time . tv_nsec - start_time . tv_nsec < 0) 
    nanosecs = (((long )end_time . tv_sec) - ((long )start_time . tv_sec) - 1) * 1000000000L + (1000000000L + ((long )end_time . tv_nsec) - ((long )start_time . tv_nsec));
   else 
    nanosecs = (((long )end_time . tv_sec) - ((long )start_time . tv_sec)) * 1000000000L + (((long )end_time . tv_nsec) - ((long )start_time . tv_nsec));
  return nanosecs;
}
void simple_sgemm_tt(const int M,const int N,const int K,const float alpha,const float *A,const int LDA,const float *B,const int LDB,const float beta,float *C,const int LDC);
void simple_sgemm_tn(const int M,const int N,const int K,const float alpha,const float *A,const int LDA,const float *B,const int LDB,const float beta,float *C,const int LDC);
void tiled_sgemm_tt(const int M,const int N,const int K,const float alpha,const float *A,const int LDA,const float *B,const int LDB,const float beta,float *C,const int LDC);
...
#pragma omp parallel for private (inner) reduction (+:sum) firstprivate (LDA,LDB)
//#pragma rose_outline
...
#pragma omp parallel for private (C_row,C_col)
//#pragma rose_outline
          for (int col = 0; col <= 15; col += 1) {
// END BARRIER
// - - - - - - - - - - - - - - - - - - - -
