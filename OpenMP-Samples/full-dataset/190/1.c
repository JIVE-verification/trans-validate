...
int dummyMethod3();
int dummyMethod4();
clock_t clock() __attribute__((no_throw)) ;

void readfile(FILE *infile,double *val,int *col_idx,int *row_ptr,int *dangling_idx,int nodes,int edges)
{
  int temp1;
  int temp2;
  int i;
  ...
  int k;
  char buffer[100];
// Array for storing lines that we have to skip
  int *FromNode = (int *)(malloc(edges * sizeof(int )));
//array for storing the FromNodeId-values
  int *ToNode = (int *)(malloc(edges * sizeof(int )));
// array for storing the ToNodeId-values
  int *num_out_links = (int *)(malloc(nodes * sizeof(int )));
// no. of outbound links from each webpage
// Collecting the FromeNodeId and ToNodeId data
  i = 0;
  while(fgets(buffer,(sizeof(buffer)),infile) != ((void *)0)){
...
//Collecting total number of FromNode links for each website, and total number of nonzero values
  int nz = 0;
  int z = 0;
// no. of nonzero elements and number of zero elements
/*The z-counter is for sizing the danging-webpages index array later*/
  int temp_counter;
// Counter k
  dummyMethod1();
  
#pragma omp parallel for private (i,j) reduction (+:nz,z) firstprivate (edges)
//#pragma rose_outline
  for (i = 0; i <= nodes - 1; i += 1) {
    num_out_links[i] = 0;
// Initialize L;
    for (j = 0; j <= edges - 1; j += 1) {
...
// end of outer loop
  ...
  dummyMethod2();
// If there are any dangling webpages
  if (z != 0) {
...
void spmm(int nodes,double *val,int *col_idx,int *row_ptr,double *x_old,double *spmm_result,double d)
{
/*Performing the Sparse Matrix vector multiplictation A*x^(k-1)
  ...
  for (i = 0; i <= nodes - 1; i += 1) {
    spmm_result[i] = 0;
// Initilize
    for (j = row_ptr[i]; j <= row_ptr[i + 1] - 1; j += 1) {
...
void PageRank_iterations(double *val,int *col_idx,int *row_ptr,int *dangling_idx,int nodes,double d,double W_sum,double *x_old,double *x_new,double *spmm_result,double one_over_N)
{
...
#pragma omp parallel for firstprivate (nodes,d,W_sum,one_over_N)
//#pragma rose_outline
...
int main(int narg,char **argv)
{
  ...
  int nodes;
  int edges;
// Nodes: total number of webpages (N),
...
// Skip the second line
  fscanf(infile,"%*s %*s %d %*s %d",&nodes,&edges);
...
// array for storing non-zero values
  col_idx = ((int *)(malloc(edges * sizeof(int ))));
// array for storing the column index of each nonzero value in val
  row_ptr = ((int *)(malloc(nodes * sizeof(int ))));
// array for storing the indices at which new rows start in val
  dangling_idx = ((int *)(malloc(nodes * sizeof(int ))));
// array for storing dangling webpage indices, if any
...
//#pragma rose_outline
  ...
  dummyMethod4();
  PageRank_iterations(val,col_idx,row_ptr,dangling_idx,nodes,d,W_sum,x_old,x_new,spmm_result,one_over_N);
  dummyMethod3();
  for (int i = 0; i <= nodes - 1; i += 1) {
    printf("%lf | %lf\n",x_old[i],x_new[i]);
  }
