 ...
 * Compile with "mpicc gauss_o_2.c -o gauss_o_2.out" 
 */
/* This is aprogram running Gaussian elimination via OpenMP using 2 threads.
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
volatile float A[5000][5000];
volatile float B[5000];
volatile float X[5000];
/* A * X = B, solve for X */
/* junk */
...
/* Normalization row, and zeroing
			* element row and col */
  float multiplier;
  printf("Computing Parallel via OpenMP.\n");
/* Gaussian elimination */
  for (norm = 0; norm <= N - 1 - 1; norm += 1) {
/* Using OpenMP with 2 threads runing the for loop and having a private parameter 'norm' initiated with the value from parent thread  */
    dummyMethod1();
